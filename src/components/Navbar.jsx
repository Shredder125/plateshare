    ...
    <nav className="backdrop-blur-md bg-gradient-to-r from-gray-950/80 via-gray-900/80 to-black/80 text-white shadow-[0_0_20px_rgba(255,165,0,0.1)] sticky top-0 z-50 border-b border-gray-800">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          <Link to="/" className="relative font-medium text-gray-300 hover:text-orange-400 transition duration-300 after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-gradient-to-r after:from-orange-500 after:to-yellow-500 hover:after:w-full after:transition-all after:duration-300">
            Home
          </Link>

          <Link to="/available-foods" className="relative font-medium text-gray-300 hover:text-orange-400 transition duration-300 after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-gradient-to-r after:from-orange-500 after:to-yellow-500 hover:after:w-full after:transition-all after:duration-300">
            Available Foods
          </Link>

          {!isLoggedIn ? (
            <Link to="/login" className="px-5 py-2 rounded-full font-semibold text-black bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 hover:from-orange-400 hover:to-yellow-400 transition-all duration-300 hover:scale-105 shadow-md">
              Login
            </Link>
          ) : (
            <div className="relative group">
              <img src={user.profilePic} alt="Profile" className="w-10 h-10 rounded-full border-2 border-orange-400 cursor-pointer" />
        ...
        <button className="md:hidden text-orange-400 text-2xl focus:outline-none" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          {isMenuOpen ? <FaTimes /> : <FaBars />}
        </button>
      </div>

      <div className={`md:hidden transform transition-all duration-500 ease-in-out origin-top ${isMenuOpen ? "opacity-100 scale-y-100 max-h-[500px]" : "opacity-0 scale-y-0 max-h-0"} bg-gray-950 border-t border-gray-800 px-6 py-4 space-y-3 flex flex-col`}>
        ...
        {!isLoggedIn ? (
          <Link to="/login" onClick={() => setIsMenuOpen(false)} className="px-5 py-2 rounded-full font-semibold text-center text-black bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 hover:from-orange-400 hover:to-yellow-400 transition-all duration-300 shadow-md">
            Login
          </Link>
        ) : (
